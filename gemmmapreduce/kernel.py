...
import torch
import triton
from core import check_equality, check_speed
...
def proj_fold_acc_kernel(
        ...
        blocks):
    # scaffolding
    #  (set up local pointers, indices, et.c., given shapes and blocks)
    ptrs = ...
    masks = ...
    
    local_accum = identity
    
    for slice in slicegroup:
        # compute H
        h = ...
    
    # fold H into local aggregate
    local_accum = fold(h)

    # Atomically update 
    
    grab lock
    global_accum = load(accum_ptr at slices with mask)
    new_accum = binary_op(global_accum, local_accum)
    store(accum_ptr at slices with mask, new_accum)
    release lock
...
def proj_fold_acc_kernel(
    ...
    tl.atomic_add(n_ptr + mixs, n, mask=mmask & (n != 0.0))
...
class XEntropy(torch.autograd.Function):
    @staticmethod
...
def kernel_xentropy(p, t, c):
    ap, an = XEntropy.apply(p, t, c)
    return (ap - an).to(p.dtype)
...
if __name__ == '__main__':
    ...
    def double_xentropy(p, t, c):
        return regular_xentropy(p.to(dtype=torch.double), t.to(dtype=torch.double), c).to(dtype)
    ...
    print(kernel_xentropy(pred, trg, true))
    print(regular_xentropy(pred, trg, true))
    print(double_xentropy(pred, trg, true))
    
    print()
    print('kernel - double')
    check_equality(kernel_xentropy, double_xentropy, inputs, mock)
    print()
    print('regular - double')
    check_equality(regular_xentropy, double_xentropy, inputs, mock)
    print()
    print('kernel - regular')
    check_equality(kernel_xentropy, regular_xentropy, inputs, mock)
    print(check_speed(kernel_xentropy, inputs, mock))
    print(check_speed(regular_xentropy, inputs, mock))
